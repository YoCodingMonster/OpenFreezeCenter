...
def config_file_write(u, v, v_adv):
	...
	if v != -1:
		os.system("x-terminal-emulator -e 'bash -c \"sudo nohup python3 ${pkgdir}write_EC.py; exec bash\"'")
...
lable2 = Label(window, text = "Version 1.4", fg = 'black' if mode == 0 else 'white', bg = 'black' if mode == 1 else 'light grey', font=("Helvetica", 12))
...
def credits():
	...
	lable = Label(newWindow, text = "Rob Oudendijk", fg = 'red', bg = 'black' if mode == 1 else 'light grey', font=("Helvetica", 16))
	lable.place(x = 125, y = 10)
	lable = Label(newWindow, text = "-- Saperate Monitoring and Battery panels", fg = 'black' if mode == 0 else 'white', bg = 'black' if mode == 1 else 'light grey', font=("Helvetica", 12))
	lable.place(x = 10, y = 40)
	lable = Label(newWindow, text = "-- Dark Mode", fg = 'black' if mode == 0 else 'white', bg = 'black' if mode == 1 else 'light grey', font=("Helvetica", 12))
	lable.place(x = 10, y = 60)
	lable = Label(newWindow, text = "Thankyou so much for being a part of this project!", fg = 'grey' if mode == 0 else 'white', bg = 'black' if mode == 1 else 'light grey', font=("Helvetica", 10))
	lable.place(x = 50, y = 180)
	canvas1.pack(fill = BOTH, expand = 1)
	return

button_credits = Button(window, text = "Credits", width = 20, fg = 'black' if mode == 0 else 'white', bg = 'black' if mode == 1 else 'light grey', command = credits)
...
button_github = Button(window, text = "Github Project", width = 20, fg = 'black' if mode == 0 else 'white', bg = 'black' if mode == 1 else 'light grey', command = GitHub)
...
def install():
    os.system("x-terminal-emulator -e 'bash -c \"sudo install -Dm 644 etc/modprobe.d/GUI-MSI-DC-L-ec_sys.conf \"${pkgdir}/etc/modprobe.d/GUI-MSI-DC-L-ec_sys.conf\" && sudo install -Dm 644 etc/modules-load.d/GUI-MSI-DC-L-ec_sys.conf \"${pkgdir}/etc/modules-load.d/GUI-MSI-DC-L-ec_sys.conf\"; exec bash\"'")
...
def monitering():
	...
	os.system("x-terminal-emulator -e 'bash -c \"sudo nohup python3 ${pkgdir}monitor.py; exec bash\"'")
	return

button_GitHub = Button(window, text = "Monitering", width = 20, fg = 'black' if mode == 0 else 'white', bg = 'black' if mode == 1 else 'light grey', command = monitering)
